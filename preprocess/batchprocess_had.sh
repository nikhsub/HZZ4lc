...
OUTPUT_DIR="/uscms/home/nvenkata/nobackup/higgs+c/ML_scripts/files/training/hplusc_had_train_0403"  # Set your output directory for .pkl files
...
for file_path in "$INPUT_DIR"/*.root; do
  ...
  python process_had.py -d "$mod_file_path" -st "$save_tag" -s "$START_EVT" -e "$END_EVT"
